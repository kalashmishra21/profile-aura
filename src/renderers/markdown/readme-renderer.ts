...
export async function renderReadme(context: RenderContext): Promise<RenderResult> {
  ...
  // ── 4. CURRENT PROJECT ───────────────────────────────────────────────────
  //    Metadata of repository currently using Profile Aura
  if (context.config.project?.showCurrentProject !== false) {
    const currentProjectWidget = WidgetRegistry.getWidget('current-project');
    if (currentProjectWidget) {
      const content = await currentProjectWidget.render(context);
      if (content) markdownParts.push(content);
    }
  }

  // ── 5. FEATURED PORTFOLIO ─────────────────────────────────────────────────
  //    Premium repository cards (max 4, real descriptions only)
  ...
  // ── 7. LIVE PREVIEW ──────────────────────────────────────────────────────
  //    Showcase of generated output components
  if (context.config.project?.showPreview !== false) {
    let previewMd = `<div align="center">\n\n### // LIVE PREVIEW\n\n`;
    previewMd += `<p align="center"><i>High-fidelity components generated by Profile Aura</i></p>\n\n`;
    previewMd += `#### Hero Preview\n`;
    previewMd += `<img src="${assetsDir}/hero.svg" alt="Hero Preview" width="100%" />\n\n`;
    previewMd += `#### Overview Preview\n`;
    previewMd += `<img src="${assetsDir}/overview.svg" alt="Overview Preview" width="100%" />\n\n`;
    previewMd += `#### Metrics Preview\n`;
    previewMd += `<img src="${assetsDir}/metrics.svg" alt="Metrics Preview" width="100%" />\n\n`;
    previewMd += `</div>`;
    markdownParts.push(previewMd);
  }
