...
export const currentProjectWidget: WidgetDefinition = {
  ...
  render: async (context: RenderContext) => {
    ...
    if (projectConfig?.showCurrentProject === false) {
      return '';
    }
    ...
    // Match repo by featuredRepositories[0], configured repoName, username, profile-aura/readme-generator, or fallback to first repo
    const targetName = projectConfig?.featuredRepositories?.[0] || context.config.github.username || context.data.username;
    ...
    const latestRelease = (repo as any).latestRelease; // if available
    ...
    if (latestRelease) metaBadgeParts.push(`🏷️ <b>${latestRelease}</b> Release`);
